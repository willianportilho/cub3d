...
/*   Updated: 2022/12/26 19:15:49 by wportilh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Retorna um novo array (vetor) alocado com malloc (a memória do array
é LIBERADA, antes não era).
   start define de qual index a cópia comecará
   Pelo menos por enquanto, estou pegando o game.map e fazendo o novo array
a partir do mapa, depois dos elementos*/
...
{
	/* estou colocando bastante informações por enquanto para facilitar 
	no desenvolvimento*/
	printf("Error\n%s\n", message);
	clean_exit(game);
	exit(-1);
...
void	normalization(t_game *game)
{
	int		i;
	int		j;
	int		greather_line;
